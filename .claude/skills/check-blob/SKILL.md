---
name: check-blob
description: Verify every case-data file is wired into all three Phase-2 allowlists, then (re-)upload the case data to the private Vercel Blob store. Use after merging a new case, or any time you suspect the blob is stale / a case 404s live. Confirms the "Upload data to Blob" Action succeeds.
---
...
(b) actually uploaded to the blob. This skill checks (a) and does (b).
...
3. **Trigger the upload Action** (`.github/workflows/upload-blob.yml`, needs the repo secret
   ...
   - `mcp__github__actions_run_trigger` with `method: "run_workflow"`, `workflow_id: "upload-blob.yml"`, `ref: "main"`.

4. **Confirm it succeeded.** Poll the run's jobs and verify the `Run node scripts/upload-to-blob.mjs`
   step `conclusion: "success"`:
   - `mcp__github__actions_list` → `list_workflow_runs` for `upload-blob.yml` to get the newest run id.
   - `mcp__github__actions_list` → `list_workflow_jobs` for that run id; check the `upload` job's steps.
