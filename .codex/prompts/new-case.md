...
<!--
  ...
  `/new-case` inside Codex. Ported from `.claude/skills/new-case/SKILL.md` — keep the two in sync.
-->

Build and ship a new RT Image Matching Trainer case end to end — source free CC-BY DICOM from the
NCI Imaging Data Commons, write a reproducible generator, wire it into `trainer.html`, update the
three Phase-2 allowlists, build-verify, and open a draft PR (plus the post-merge Blob upload).

This repo has shipped ~8 cases through the same pipeline. Follow it so nothing is half-plumbed.
Read `AGENTS.md` first — it documents every existing generator and the exact wiring points.
...
- For picking among candidate patients, building + QC-rendering several in parallel and judging the
  stills works well (see how the sarcoma/PTV cases were chosen).
...
review them before wiring. Re-run is in-place / idempotent.
...
```
Must minify clean (terser parse + served-output gate invariants). **Codex has no pre-push hook**, so
run this (and the allowlist check) yourself before pushing — see `AGENTS.md` › "Working with Codex".

## 6 — Document, commit, push, draft PR

- Add a generator bullet to `AGENTS.md` **and** `CLAUDE.md` (and the case to the Cases list) — keep
  the two guides in sync.
...
prompt (or trigger the "Upload data to Blob" Action from `main` and confirm success). Until then the
new case 404s live.
