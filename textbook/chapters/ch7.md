...
> **Key Point:** "Dose" always comes with a *location*. A prescription of 200 cGy means 200 cGy *at the prescription point or isodose line* — not everywhere in the beam [2].
...
- **SAD (Source-to-Axis Distance) setup**, also called **isocentric**. You fix the distance from the source to the **isocenter**, which sits *inside* the patient at the target. The skin is somewhere above it. SAD setups use **TMR** (or TPR) tables. This is the standard for modern linac treatments because you can rotate the gantry all the way around without moving the patient [1].
...
> **Key Point:** **Equivalent square = 2ab / (a + b)** (the same as 4 × Area ÷ Perimeter), where *a* and *b* are the rectangle's sides [1].
...
> **Key Point:** **PDD = (dose at depth d ÷ dose at d_max) × 100.** PDD belongs to **SSD** setups. PDD goes *up* with higher energy, larger field size, and longer SSD; it goes *down* with depth [2].
...
- **TAR (Tissue-Air Ratio).** An older ratio (dose in tissue ÷ dose in air at the same point). You should recognize the name, but TMR has largely replaced it for high-energy beams [2].
...
Recall from Chapter 3 (Figure 3.3) that dose rate falls with the **square** of the distance from the source: **D₂ = D₁ × (d₁/d₂)²**. In dose calculation this is how you correct for treating at a distance other than the calibration distance [2].
...
- **Phantom (patient) scatter, S_p.** Extra dose scattered from the **patient's own tissue**. It depends on the field size at the patient. Together, **S_cp = S_c × S_p** is the total scatter factor [1].
...
A **monitor unit (MU)** is how a linac measures its output — roughly, "how long the beam stays on." The MU calculation asks: *given everything above, how many MUs deliver the prescribed dose?* The logic is always the same — **divide the dose you want by the dose you get per MU**, after accounting for every factor [1].
...
> **Key Point:** **Gap = (L/2) × (d / SSD)** for each field, where L is the field length, d is the junction depth, and SSD is the source-to-surface distance. Add the two fields' contributions for the total skin gap [2, 4].
...
A handy rule of thumb is that **R₉₀ ≈ E ÷ 3.2** (in cm, for energy E in MeV), and the practical range **Rp ≈ E ÷ 2**. Both depths get **deeper as energy rises** [2].
...
> **Key Point:** On a DVH, a **good target** curve holds near 100% volume and then falls off a cliff right at the prescription dose; a **well-spared organ at risk** drops off early, staying low and to the left [2, 4].
